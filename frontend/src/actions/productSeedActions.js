...
export const listSeedProducts = () => async (dispatch) => {
  ...
  try {
    ...
    const { data } = await axios.get(
      `${process.env.REACT_APP_API_URL}/api/seeds`
    );
...
export const listSeedProductsDetails = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(
      `${process.env.REACT_APP_API_URL}/api/seeds/${id}`
    );
...
export const updateSeedProducts = (seed) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.put(
      `${process.env.REACT_APP_API_URL}/api/seeds/${seed._id}`,
      seed,
      config
    );
...
export const createProductReview =
  (productId, review) => async (dispatch, getState) => {
    try {
      ...
      await axios.post(
        `${process.env.REACT_APP_API_URL}/api/seeds/${productId}/reviews`,
        review,
        config
      );
